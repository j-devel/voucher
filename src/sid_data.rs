...
pub fn content_comp_permissive(a: &[u8], b: &[u8]) -> bool {
    let mask = |v: Vec<u8>| {
        // Mask CBOR `CborType::{StringAsBytes,Bytes}` to be permissive
        move |(i, x): (usize, &u8)| {
            if i == 0 { *x } else {
                match v[i - 1] { // "maybe" SID delta
